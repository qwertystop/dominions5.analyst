defmodule Parser.Readers.DomMap do
  @type subspec :: {atom, pos_integer}

  @spec read!(Enumerable.t, subspec, subspec) :: Parser.collection
  def read!(input_bytestream, {key_type=:integer, key_size}, {val_type, val_size}) do
  ...
  defp read_key_int(source, key_type, key_size, val_type, val_size,
                    acc_map\\%{}, acc_size\\0) do
    key = delegate_reader(source, key_type, key_size)
    if key == 0 do
      {acc_map, :map, acc_size + key_size}
    else
      read_val_int(source, key_type, key_size, val_type, val_size, acc_map, acc_size + key_size, key)
    end
  end

  defp read_val_int(source, key_type, key_size, val_type, val_size,
                    acc_map, acc_size, last_key) do
    val = delegate_reader(source, val_type, val_size)
    read_key_int(source, key_type, key_size, val_type, val_size,
